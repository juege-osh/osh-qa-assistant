...
(
  ...
  `icon_path`       varchar(300) DEFAULT NULL COMMENT '图标存放路径,格式如:resources/type/20230523/123.jpg',
  `active_split_strategy`    varchar(50)  DEFAULT NULL COMMENT '当前生效切分策略',
  `active_split_config_json` text         DEFAULT NULL COMMENT '当前生效切分参数快照(json)',
  `active_experiment_id`     bigint       DEFAULT NULL COMMENT '当前生效实验版本id',
  `active_experiment_name`   varchar(150) DEFAULT NULL COMMENT '当前生效实验版本名称',
  `created_time`    datetime     NOT NULL DEFAULT current_timestamp() COMMENT '创建时间',
  `modified_time`   datetime     NOT NULL DEFAULT current_timestamp() COMMENT '修改时间',
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`)
