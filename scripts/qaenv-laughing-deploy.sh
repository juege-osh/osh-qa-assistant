...
if [ -z "$DEPLOY_PASSWORD" ]; then
  echo "QAENV_DEPLOY_PASSWORD is required" >&2
  exit 1
fi
...
sshpass -p "$DEPLOY_PASSWORD" ssh -o StrictHostKeyChecking=accept-new -p "$DEPLOY_PORT" "$DEPLOY_USER@$DEPLOY_HOST" "
...
sshpass -p "$DEPLOY_PASSWORD" rsync -az --inplace \
...
sshpass -p "$DEPLOY_PASSWORD" rsync -az \
  -e "ssh -o StrictHostKeyChecking=accept-new -p $DEPLOY_PORT" \
...
sshpass -p "$DEPLOY_PASSWORD" rsync -az --delete \
  -e "ssh -o StrictHostKeyChecking=accept-new -p $DEPLOY_PORT" \
